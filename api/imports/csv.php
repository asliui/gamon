...
// api/imports/csv.php — Admin CSV import (categories, areas, reports).
...
if (!isset($_FILES['file']) || $_FILES['file']['error'] !== UPLOAD_ERR_OK) {
    Response::json(['ok' => false, 'error' => 'CSV file is required'], 400);
}
...
if ($ext !== 'csv') {
    Response::json(['ok' => false, 'error' => 'Only .csv files are allowed'], 400);
}
...
if ($mime === false || !in_array($mime, $allowed, true)) {
    Response::json(['ok' => false, 'error' => 'Invalid CSV file type'], 400);
}
...
$rows = ImportService::parseCsvPayload($raw, $entity);
